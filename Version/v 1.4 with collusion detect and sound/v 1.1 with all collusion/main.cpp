...
void display(void)
{
    ...
    else
    {
        ...
        for (int i = 0; i < enemy_count; i++)
        {
            ...
            if ((UFO_1_X_pos[i] - 10 >= spaceship_X_min - 10 && UFO_1_X_pos[i] + 10 <= spaceship_X_min + spaceship_X_max && spaceship_Y_max <= UFO_1_Y_pos[i] && spaceship_Y_max >= UFO_1_Y_pos[i] - 0.2) || (UFO_1_X_pos[i] + UFO_1_X2_pos >= spaceship_X_min && spaceship_Y_max <= UFO_1_Y_pos[i] && spaceship_Y_max >= UFO_1_Y_pos[i] - 0.2))
            {
                spaceship_clash++;
                // Explosion Sound
                PlaySound(TEXT("F:/Study/Semester 9/Computer Graphics/Project/Space Invaders/assets/sound/Explosion3.wav"), NULL, SND_ASYNC);

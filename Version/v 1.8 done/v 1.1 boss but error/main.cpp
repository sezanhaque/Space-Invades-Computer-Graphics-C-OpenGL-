...
float boss_X_pos[1] = {450};
float boss_Y_pos[1] = {0};
float boss_X2_pos = 240;
float boss_Y2_pos = 240;

int delay_time = 0;
int delay_time_2 = 0;
int delay_time_boss = 0;
...
void myInit(void)
{
    ...
    imageloader *boss = loadBMP("F:/Study/Semester 9/Computer Graphics/Project/Space Invaders/assets/main_boss.bmp");
    ...
    imageloader *level = loadBMP("F:/Study/Semester 9/Computer Graphics/Project/Space Invaders/assets/level_1.bmp");
...
{
    glEnable(GL_TEXTURE_2D);
    glBindTexture(GL_TEXTURE_2D, texture_level); // This will let OpenGl know which texture we want to use
...
void main_boss(float UFO_1_X_pos, float UFO_1_Y_pos)
{
    glEnable(GL_TEXTURE_2D);
    glBindTexture(GL_TEXTURE_2D, texture_boss); // This will let OpenGl know which texture we want to use
    ...
    // OpenGL picks the coordinates of an image starting from top left, so (0,0) will be on X(min) Y(max) position
    glTexCoord2i(0, 0); // picking up the top left coordinates of the image, X(min) Y(max)
    glVertex2i(UFO_1_X_pos, UFO_1_Y_pos + 122);

    glTexCoord2i(1, 0); // picking up the top right coordinates of the image, X(max) Y(max)
    glVertex2i(UFO_1_X_pos + 240, UFO_1_Y_pos + 122);

    glTexCoord2i(1, 1); // picking up the bottom right coordinates of the image, X(max) Y(min)
    glVertex2i(UFO_1_X_pos + 240, UFO_1_Y_pos);
...
void display(void)
{
    ...
    else if (start == true && gameover == false)
    {
        ...
        else
        {
            if (!pause)
            {
                ...
                // Giving limit to enemy
                if(enemy_count < 2 && enemy_2_count < 2)
                {
                    if (delay_time > 2000)
                    {
                ...
                else
                {
                    ...
                    for (int i = 0; i < boss_count; i++)
                    {
                        ...
                        {
                            boss_Y_pos[i] += 0.2;
                            main_boss(boss_X_pos[i], boss_Y_pos[i]);
                        }
                        if (boss_Y_pos[i] > 500)
                        {
                            boss_Y_pos[i] -= 0.2;
                ...
                for (int i = 0; i < fire_count; i++)
                {
                    for (int j = 0; j < boss_count; j++)
                    {
                        // Perfect collision
                        // If U_X_min < B_X_min < U_X_max      and     U_Y_min < B_Y_max < U_Y_max
                        if ((boss_X_pos[j] - 10 < bullet_X_arr[i] && bullet_X_arr[i] < boss_X_pos[j] + 240 && boss_Y_pos[j] < bullet_Y_arr[i] + 25 && bullet_Y_arr[i] + 25 < boss_Y_pos[j] + 240) || (boss_X_pos[j] - 10 < bullet_2_X_arr[i] && bullet_2_X_arr[i] < boss_X_pos[j] + 240 && boss_Y_pos[j] < bullet_2_Y_arr[i] + 25 && bullet_2_Y_arr[i] + 25 < boss_Y_pos[j] + 240))
                        {
                            boss_clash_count++;
                            clash_count++;
                            fire_count--;
                            cout << boss_clash_count << endl;

                            if (boss_clash_count > 10)
                            {
                                boss_count--;
    ...
    else if (start == false && level_1_completed == true)
    {
        level_1();
        // cout << "Entered" << endl;
    }
    // else if (start == false && level_1_completed == true && gameover == true)
    // {
    //     level_1();
    //     cout << "Entered" << endl;
    // }
